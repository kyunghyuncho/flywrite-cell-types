...
def eval_heldout_ll(
    ...
    """E_{q_i q_j}[log Bern(y | sigma(u_i·u_j+b))] averaged over held-out pairs."""
    alpha = torch.softmax(q_logits, dim=-1)
    e_prob_kk = torch.sigmoid(u_left @ u_right.T + bias)  # K x K
    src = held["src"]
    tgt = held["tgt"]
    ...
    for start in range(0, len(src), chunk):
        ...
        # Soft expected edge probability: alpha_i^T B alpha_j
        ai = alpha[src[sl]]
        aj = alpha[tgt[sl]]
        p = ((ai @ e_prob_kk) * aj).sum(dim=-1).clamp(1e-6, 1 - 1e-6)
...
def train(args: argparse.Namespace) -> dict:
    ...
    try:
        for epoch in range(args.epochs):
            ...
            for step in tqdm(range(n_minibatches), desc=f"lv epoch {epoch}"):
                ...
                # Expected complete log-likelihood with held-out pairs removed.
                # Weight each (k,k') by sum_{kept ij} q_i(k) q_j(k') * [y log p + ...]
                w_edge = q_i.T @ (cc * keep.to(dtype)) @ q_j
                w_non = q_i.T @ ((1.0 - cc) * keep.to(dtype)) @ q_j
                e_prob = torch.sigmoid(u_left @ u_right.T + bias)
                obj = (w_edge * log_clamp(e_prob)).sum() + (w_non * log_clamp(1.0 - e_prob)).sum()
                ...
                obj = obj * (idx_i.numel() * idx_j.numel()) / keep.sum().clamp(min=1).to(dtype)
                loss = -obj
            ...
            val_ll = eval_heldout_ll(q_logits, u_left, u_right, bias, held, device, dtype)
    ...
    print(f"Saved {prefix}_* ; val_ll={val_ll:.6f}")
    return metrics
